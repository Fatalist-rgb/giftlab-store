...
__export(logic_exports, {
  ...
  DesignStateInvalid: () => DesignStateInvalid,
  FreeDefaultViolation: () => FreeDefaultViolation,
  assessPhoto: () => assessPhoto,
  buildCutContour: () => buildCutContour,
  characterLayerZ: () => characterLayerZ,
  computePrice: () => computePrice,
  computeWithdrawalRight: () => computeWithdrawalRight,
  ...
  optionValueZ: () => optionValueZ,
  optionZ: () => optionZ,
  parseDesignState: () => parseDesignState,
  parseProductSchema: () => parseProductSchema,
  photoStatusZ: () => photoStatusZ,
  physicalZ: () => physicalZ,
  pricingRulesZ: () => pricingRulesZ,
  productSchemaZ: () => productSchemaZ,
  quantityTierZ: () => quantityTierZ,
  safeParseProductSchema: () => safeParseProductSchema,
  selectionDelta: () => selectionDelta,
  textFieldZ: () => textFieldZ,
  textValueZ: () => textValueZ,
  unitPriceForQuantity: () => unitPriceForQuantity,
  variantZ: () => variantZ
});
...
// src/schema/types.ts
var localizedTextZ = external_exports.record(external_exports.string());
var variantZ = external_exports.object({
...
var characterLayerZ = external_exports.object({
  id: external_exports.string().min(1),
  label: localizedTextZ.optional(),
  zIndex: external_exports.number().int(),
  variants: external_exports.array(variantZ).min(1)
});
var faceZoneZ = external_exports.object({
  ...
  maskAssetKey: external_exports.string().min(1),
  minResolutionPx: external_exports.object({
    w: external_exports.number().int().positive(),
    h: external_exports.number().int().positive()
  }),
  transforms: external_exports.array(external_exports.enum(["move", "scale", "rotate"])).default([]),
  // where the face sits in the layer stack (under the top acrylic outline, above the body)
  zIndex: external_exports.number().int().default(50)
});
var textFieldZ = external_exports.object({
  id: external_exports.string().min(1),
  label: localizedTextZ.optional(),
  maxLen: external_exports.number().int().positive(),
  fonts: external_exports.array(external_exports.string()).min(1),
  colors: external_exports.array(external_exports.string()).min(1),
  placement: external_exports.string().default("figure"),
  zIndex: external_exports.number().int().default(100)
});
var optionValueZ = external_exports.object({
  id: external_exports.string().min(1),
  label: localizedTextZ.optional(),
  priceDelta: external_exports.number().int().default(0)
});
var optionZ = external_exports.object({
  id: external_exports.string().min(1),
  label: localizedTextZ.optional(),
  values: external_exports.array(optionValueZ).min(1),
  default: external_exports.string().optional()
});
var quantityTierZ = external_exports.object({
  minQty: external_exports.number().int().positive(),
  unitPrice: external_exports.number().int().nonnegative()
});
var pricingRulesZ = external_exports.object({
  base: external_exports.number().int().nonnegative(),
  currency: external_exports.string().length(3),
  quantityLadder: external_exports.array(quantityTierZ).min(1)
});
var physicalZ = external_exports.object({
  heightMm: external_exports.number().positive(),
  magneticBacking: external_exports.boolean().default(false),
  material: external_exports.string().optional()
});
var cutContourZ = external_exports.object({
  source: external_exports.enum(["composite", "silhouette", "asset"]).default("composite"),
  offsetMm: external_exports.number().nonnegative().default(3),
  spotName: external_exports.string().default("CutContour")
});
var productSchemaZ = external_exports.object({
  id: external_exports.string().min(1),
  medusaProductId: external_exports.string().min(1),
  version: external_exports.number().int().positive(),
  status: external_exports.enum(["draft", "published", "archived"]),
  // native pixel dimensions of the artwork — the coordinate space faceZone/text live in
  canvasPx: external_exports.object({ w: external_exports.number().int().positive(), h: external_exports.number().int().positive() }),
  characterLayers: external_exports.array(characterLayerZ).min(1),
  faceZone: faceZoneZ,
  textFields: external_exports.array(textFieldZ).default([]),
  options: external_exports.array(optionZ).default([]),
  physical: physicalZ,
  constraints: external_exports.object({ rules: external_exports.array(external_exports.unknown()).default([]) }).default({ rules: [] }),
  pricingRules: pricingRulesZ,
  cutContour: cutContourZ
});

// src/errors.ts
var ConstructorError = class extends Error {
  constructor(message) {
    super(message);
    this.name = new.target.name;
  }
};
var FreeDefaultViolation = class extends ConstructorError {
  constructor(totalDelta) {
    super(`default selection carries a non-zero price delta (${totalDelta})`);
    this.totalDelta = totalDelta;
  }
};
var DesignStateInvalid = class extends ConstructorError {
};

// src/schema/validate.ts
function defaultSelectionPriceDelta(schema) {
  let total = 0;
...
function parseProductSchema(input) {
...
function safeParseProductSchema(input) {
...
// src/design-state/types.ts
var faceLayerZ = external_exports.object({
  uploadedPhotoId: external_exports.string().min(1),
  x: external_exports.number(),
  y: external_exports.number(),
  scale: external_exports.number().positive(),
  rotation: external_exports.number()
});
var textValueZ = external_exports.object({
  fieldId: external_exports.string().min(1),
  value: external_exports.string(),
  font: external_exports.string().optional(),
  color: external_exports.string().optional()
});
var photoStatusZ = external_exports.enum(["ready", "deferred", "processing", "failed"]);
var designStateZ = external_exports.object({
  productSchemaId: external_exports.string().min(1),
  schemaVersion: external_exports.number().int().positive(),
  /** layerId -> variantId */
  characterSelections: external_exports.record(external_exports.string()),
  /** null while the customer defers the photo (order now, send it later) */
  faceLayer: faceLayerZ.nullable(),
  textValues: external_exports.array(textValueZ).default([]),
  /** optionId -> valueId (empty for this product — every option is free) */
  selectedOptions: external_exports.record(external_exports.string()).default({}),
  quantity: external_exports.number().int().positive().default(1),
  photoStatus: photoStatusZ.default("ready")
});

// src/design-state/validate.ts
function parseDesignState(input, schema) {
...
function crossCheck(design, schema) {
  const problems = [];
...
function unitPriceForQuantity(rules, qty) {
...
function selectionDelta(schema, design) {
  let delta = 0;
  for (const [layerId, variantId] of Object.entries(design.characterSelections)) {
    const layer = schema.characterLayers.find((l) => l.id === layerId);
...
// src/legal/withdrawal.ts
function computeWithdrawalRight(design) {
...
function assessPhoto(photo, schema) {
