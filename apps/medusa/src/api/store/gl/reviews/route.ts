...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  if (body.orderDisplayId && body.email) {
    try {
      ...
      // display_id is filterable at runtime; the DTO typing just lags behind
      const filters = { display_id: body.orderDisplayId } as unknown as Parameters<
        IOrderModuleService['listOrders']
      >[0]
      const [order] = await orders.listOrders(filters, { relations: ['items'], take: 1 })
