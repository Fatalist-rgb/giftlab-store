...
export class Migration20260719174834 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`alter table if exists "content_page" drop constraint if exists "content_page_slug_locale_unique";`);
    this.addSql(`create table if not exists "content_page" ("id" text not null, "slug" text not null, "locale" text check ("locale" in ('pl', 'en', 'uk')) not null, "title" text not null, "body" text not null, "meta_title" text null, "meta_description" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "content_page_pkey" primary key ("id"));`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_content_page_slug" ON "content_page" ("slug") WHERE deleted_at IS NULL;`);
